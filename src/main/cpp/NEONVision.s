## Note: this is legacy code that is currently kept only for reference. It will be deleted soon.
##
...
## in a wierd dialect of AArch64, modified to fit llvm's syntax. on top of that, i'm running this
## through the C/C++ preprocessor, which necessitates using double hashes for comments.
...
##
    ...
    .globl Java_org_ftc9974_thorcore_vision_NEONVision_processYUY2ForDisplay
    .type Java_org_ftc9974_thorcore_vision_NEONVision_processYUY2ForDisplay, %function
...
processYUY2_done:
    ...
    ## like processYUY2, but it overwrites the pixel buffer so it can be used to see what the
    ## algorithm is doing. having the entire thing duplicated with a few additional instructions may
    ## seem redundant, but the whole idea behind NEONVision is to push performance to the absolute
    ## limit. running any unnecessary instructions is an unacceptable performance loss.
    ##
    ## args:
    ## JNIEnv* env -> x0
    ## jobject obj -> x1
    ...
    ## jlong highColor -> x4 (YUV order)
    ## jlong lowColor -> x5 (YUV order)
Java_org_ftc9974_thorcore_vision_NEONVision_processYUY2ForDisplay:
## begin local names
#define bufferPtr x2
#define bufferLen x3
## end local names
    ## ensure the buffer has at least one block of pixels.
    cmp bufferLen, #64
    b.ge processYUY2ForDisplay_start
    ## indicate error code -2
    mov x0, #-2
    ret
processYUY2ForDisplay_start:
    ## prefetch the first block. we do it here to maximize the time the cache controller has to read
    ## from system memory before the data is needed
    prfm PLDL1STRM, [bufferPtr]

    ## calculate a pointer to the end of the buffer, storing it in x7
#define bufferLimit x7
    add bufferLimit, bufferPtr, bufferLen
...
processYUY2ForDisplay_loop:
    ## calculate the number of remaining bytes in the buffer, storing it in x6
#define bytesRemaining x6
    subs bytesRemaining, bufferLimit, bufferPtr

    ## if there's less than one block of pixels remaining in the buffer, break the loop
    cmp bytesRemaining, #64
    b.lt processYUY2ForDisplay_done

    ## prefetch the next block of pixels
    prfm PLDL1STRM, [bufferPtr, #64]

## this label doesn't actually do anything, it just helps with organization
processYUY2ForDisplay_processBlock:
    ## v0-v3 will store pixel channels.
    ## V0 -> v0
    ## Y1 -> v1
    ## U0 -> v2
    ...
    ## this algorithm operates on "blocks" of the image. each block is 64 bytes and comprises 32 pixels.
    ## padding is not needed if the image is in 16:9 and the width is divisible by 2.
    ## the USB camera returns big-endian YUYV?
#define pixelV0 v0
#define pixelY1 v1
#define pixelU0 v2
#define pixelY0 v3
    ld4 {v0.16b, v1.16b, v2.16b, v3.16b}, [x2]
    mov v5.16b, v0.16b
    mov v0.16b, v3.16b
    mov v3.16b, v5.16b
    mov v5.16b, v1.16b
    ...
    ## tell the memory controller we'll be writing back to the buffer soon.
    prfm PSTL1STRM, [x2]
    ...
    ## channel: Y0
    cmhs v7.16b, v19.16b, v3.16b
    ## compare to low (pixel > low), storing result in v25-v28. the pixel data isn't needed after this
    ## point, which is why we can overwrite it.
    ## channel: V0
    cmhs v25.16b, v0.16b, v20.16b
    ## channel: Y1
    cmhs v26.16b, v1.16b, v21.16b
    ## channel: U0
    cmhs v27.16b, v2.16b, v22.16b
    ## channel: Y0
    cmhs v28.16b, v3.16b, v23.16b
    ...
    ## UVcmp -> v25
    ## (V0 > lowV) is already in v25.
    ## (V0 > lowV) && (highV > V0) -> v25
    and v25.16b, v25.16b, v4.16b
    ## (V0 > lowV) && (highV > V0) && (U0 > lowU) -> v25
    and v25.16b, v25.16b, v27.16b
    ## (V0 > lowV) && (highV > V0) && (U0 > lowU) && (highU > U0) -> v25
    and v25.16b, v25.16b, v6.16b

    ## Y0cmp -> v28
    ## (Y0 > lowY) is already in v28.
    ## (Y0 > lowY) && (highY > Y0) -> v28
    and v28.16b, v28.16b, v7.16b

    ## Y1cmp -> v26
    ## (Y1 > lowY) is already in v26.
    ## (Y1 > lowY) && (highY > Y1) -> v26
